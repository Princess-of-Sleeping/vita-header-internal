
#ifndef _PSP2_KERNEL_SYSMEM_UID_GUID_H_
#define _PSP2_KERNEL_SYSMEM_UID_GUID_H_

#include <shared/types.h>


#ifdef __cplusplus
extern "C" {
#endif


int SceSysmemForKernel_C2F7D8A4(void *pEntry);
int SceSysmemForKernel_8FB73A29(void *pEntry);
int SceSysmemForKernel_876A7F44(void *pEntry);
int SceSysmemForKernel_6BB6AF94(void *pEntry);
/*
          sceGUIDClose: 0x047D32F2
          sceGUIDCreate: 0x89A44858
          sceGUIDGetClass: 0xC74B0152
          sceGUIDGetName: 0xA78755EB
          sceGUIDGetName2: 0xE655852F
          sceGUIDGetObject: 0x0FC24464
          sceGUIDOpenByName: 0xD76E7452
          sceGUIDReferObject: 0x0F5C84B7
          sceGUIDReferObjectWithClass: 0x00ED6C14
          sceGUIDReferObjectWithClassLevel: 0x77066FD1
          sceGUIDReferObjectWithLevel: 0xF6DB54BA
          sceGUIDReferObjectWithSubclass: 0x72A98D17
          sceGUIDReleaseObject: 0x149885C4
          sceGUIDSetName: 0x4CFA4100
          sceUIDKernelCreate2: 0x56A13E90
          sceGUIDFindByName: 0xCEBA8031
          sceGUIDFindByNameAll: 0x4B5C85AC
          sceGUIDFindByObject: 0x95ABFDC3
          sceGUIDGetEntry: 0x45F2A59C
          sceGUIDGetEntryInfoAll: 0x2A79C51C
          sceGUIDGetEntryInfoClass: 0xE90CFD62
          sceGUIDGetObjectWithClass: 0x7ABFA9A7
          sceGUIDGetPID: 0xCF5A2311
          sceGUIDGetUIDVectorByClass: 0xEC7D36EF
          sceGUIDGetUIDVectorWeakByClass: 0x59F577E8
          sceGUIDGetVisibilityLevel: 0xC69666C3
          sceGUIDKernelCreateWithAttr: 0x53E1FFDE
          sceGUIDKernelCreateWithOpt: 0xDF0288D7
          sceGUIDOpenByGUID: 0xCF53EEE4
          sceGUIDRefer: 0xB3E2AA7A
          sceGUIDRegister: 0xAF42AAD5
          sceGUIDSet: 0xD7B323EB
          sceGUIDSetCNOA: 0x8D6AF468
          sceGUIDSetPID: 0x812E7A53
          sceGUIDSetVisibilityLevel: 0xCB8D03C0
          sceGUIDtoClass: 0x66636970
          sceUIDtoObject: 0xED221825
*/


#ifdef __cplusplus
}
#endif


#endif /* _PSP2_KERNEL_SYSMEM_UID_GUID_H_ */
